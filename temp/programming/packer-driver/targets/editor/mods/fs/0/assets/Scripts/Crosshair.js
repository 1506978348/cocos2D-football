  ...
  return {
    ...
    execute: function () {
      ...
      _export("Crosshair", Crosshair = (_dec = ccclass('Crosshair'), _dec2 = property({
        ...
        CalculateDistance() {
          let distance = Math.sqrt((this.newBall.position.x - this.node.position.x) * (this.newBall.position.x - this.node.position.x) + (this.newBall.position.y - this.node.position.y) * (this.newBall.position.y - this.node.position.y)); //if ball is within radius - hit
